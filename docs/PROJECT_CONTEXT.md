...
- **세계관 연출**: 영토 색상이 프로필 배너, 신념 문장, 성향 레이더에 통일되게 반영
...
> **프로필 UI 리디자인 진행 중 (2026-07-04)**  
> 기존 2열 HUD 레이아웃에서 **Grid 기반 구조**(`profile-main` + `profile-summaries`)로 전환 중입니다.  
> 이후 작업은 새 Grid 레이아웃을 기준으로 이어갑니다. (뷰포트 높이 최적화 1차 완료)
...
- 팔로우 시스템 + 알림
- 플레이어 프로필 패널 — **Grid 재설계 진행 중**
  - 상단: 사이드바(아바타 + 4축 성향 레이더) | 메인(닉네임·스탯·신념·경험치)
  - 하단: 대표 업적 · 활동 요약 · 영토 기록 3카드
  - `window.TERRITORY_BELIEFS` 기반 신념 HUD (엠블럼 + belief + displayName의 신념)
  - 뷰포트 높이 1차 최적화 (패널 스크롤 제거, 세로 공간 압축)
  - 명예 장식 슬롯 (프레임/칭호/휘장/오라) — 패널 하단
- 레벨/XP/명성 표시
- 권한 안내 탭
- 히스토리 탭
- 게시글 상세 화면
